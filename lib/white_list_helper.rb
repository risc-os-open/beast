module WhiteListHelper
  ...
  #
  # 2013-09-04 (ADH): If the attributes lists is empty, any attribute is allowed, except for "bad protocol" data which is still stripped.
  #
          ...
          unless tags.include?(node.name)
            ...
            if node.closing != :close
              node.attributes.delete_if do |attr_name, value|
                (!attrs.empty? && !attrs.include?(attr_name)) || (PROTOCOL_ATTRIBUTES.include?(attr_name) && contains_bad_protocols?(value))
